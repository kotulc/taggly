...
def test_config_yaml_overrides_default(config_dir, monkeypatch):
    ...
    assert registry["key"]._config.model == "yake"
...
def test_config_yaml_partial_override(config_dir, monkeypatch):
    ...
    monkeypatch.chdir(config_dir)
    registry = discover_commands()
    assert registry["ent"]._config.language == "en_core_web_sm"
    assert registry["key"]._config.model == "keybert"  # untouched default
...
def test_missing_config_yaml_uses_defaults(tmp_path, monkeypatch):
    ...
    registry = discover_commands()
    assert registry["key"]._config.model == "keybert"
