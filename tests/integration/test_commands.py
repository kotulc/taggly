...
def test_has_docstring(name, cmd):
    """Command operation method has a non-empty docstring."""
    doc = cmd.operation.__doc__
    assert doc and doc.strip(), f"{name}.operation() is missing a docstring"


@pytest.mark.parametrize("name,cmd", COMMANDS)
def test_config_fields_have_descriptions(name, cmd):
    """Every Config and Params field has a non-empty description for CLI help and API docs."""
    checked = False
    for attr in ("Config", "Params"):
        cls = getattr(cmd, attr, None)
        if cls is None:
            continue
        checked = True
        for fname, field in cls.model_fields.items():
            assert field.description and field.description.strip(), (
                f"{name}.{attr}.{fname} is missing a Field description"
            )
    if not checked:
        pytest.skip(f"{name} has no Config or Params")
...
def test_api_endpoint(name, cmd):
    """POST /{name} returns 200 with valid body and Params fields as query params."""
    payload = _sample_input(cmd.Input)
    params_cls = getattr(cmd, "Params", None)
    params = _sample_input(params_cls) if params_cls is not None else None
