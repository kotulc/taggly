...
def discover_commands(commands_dir: Path=None, app_config=None) -> Dict[str, AbstractBaseCommand]:
    ...
    Per-command Config defaults are read from config.yaml. In CLI mode each command is
    given an api_url if the API server is already running.
    """
    ...
    for file in commands_dir.rglob("*.py"):
        ...
        for _, obj in inspect.getmembers(module, inspect.isclass):
            ...
            if config_cls is not None:
                ...
                kwargs["config"] = config_cls(**raw) if raw else config_cls()

            registry[cmd_name] = obj(**kwargs)
...
def _check_api(app_config) -> str | None:
    """Return the API base URL if the server is reachable and this process is not the server."""
